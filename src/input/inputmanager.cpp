...
void InputManager::setupSeatUserConfig(const QString &userName)
{
    ...
    if (!m_userName.isEmpty()) {
        for (WSeat *seat : seats) {
            auto *config = configManager->seatUserConfig(m_userName, seat->name());
...
SeatUserDConfig *InputManager::seatUserConfig(WSeat *seat) const
{
    ...
    auto *config = DConfigManager::instance()->seatUserConfig(m_userName, seat->name());
    Q_ASSERT(config);
...
void InputManager::setupSeat(WSeat *seat)
{
    ...
    Q_ASSERT(config->isInitializeSucceeded() || config->isInitializeFailed());
