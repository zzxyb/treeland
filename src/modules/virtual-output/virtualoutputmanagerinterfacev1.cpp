                                                                   ...
                                                                   const QString &name, wl_array *outputs)
{
    ...
    auto virtuaOoutput = new VirtualOutputInterfaceV1(name, outputs, outputResource);
    s_virtualOutputs.append(virtuaOoutput);

    QObject::connect(virtuaOoutput, &QObject::destroyed, [virtuaOoutput, this]() {
        Q_EMIT q->destroyVirtualOutput(virtuaOoutput);
        s_virtualOutputs.removeOne(virtuaOoutput);
    });

    if (name.isEmpty()) {
        virtuaOoutput->sendError(TREELAND_VIRTUAL_OUTPUT_V1_ERROR_INVALID_GROUP_NAME,
                                  "Group name is empty!");
        return;
    }

    if (outputs->size < 2) {
        virtuaOoutput->sendError(TREELAND_VIRTUAL_OUTPUT_V1_ERROR_INVALID_SCREEN_NUMBER,
                                  "The number of screens applying for copy mode is less than 2!");
        return;
    }

    Q_EMIT q->requestCreateVirtualOutput(virtuaOoutput);
}
...
void VirtualOutputManagerInterfaceV1Private::get_virtual_output(Resource *resource,
                                                                const QString &name, uint32_t id)
{
    for (auto interface : std::as_const(s_virtualOutputs)) {
