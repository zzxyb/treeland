...
QString MpvVideoController::getError(int error)
{
    switch (error) {
...
{
    QMetaObject::invokeMethod(static_cast<MpvVideoController *>(ctx), &MpvVideoController::eventHandler, Qt::QueuedConnection);
}
...
void MpvVideoController::init()
{
    ...
    m_mpv = mpv_create();
    if (!m_mpv) {
        qCFatal(treelandWallpaper) << "could not create mpv context";
    }

    mpv_set_option_string(m_mpv, "hwdec", "auto");

    if (mpv_initialize(m_mpv) < 0) {
        qCFatal(treelandWallpaper) << "could not initialize mpv context";
    }

    mpv_set_wakeup_callback(m_mpv, MpvVideoController::mpvEvents, this);
    // use number of logical CPU cores
    setProperty(QStringLiteral("ad-lavc-threads"), 0);
    // render to fbo
    setProperty(QStringLiteral("vo"), QStringLiteral("libmpv"));
}

void MpvVideoController::observeProperty(const QString &property, mpv_format format, uint64_t id)
{
...
int MpvVideoController::setProperty(const QString &property, const QVariant &value)
{
    mpv_node node;
    setNode(&node, value);
...
int MpvVideoController::setPropertyAsync(const QString &property, const QVariant &value, int id)
{
    mpv_node node;
    setNode(&node, value);
    return mpv_set_property_async(m_mpv, id, property.toUtf8().constData(), MPV_FORMAT_NODE, &node);
}

QVariant MpvVideoController::getProperty(const QString &property)
{
    mpv_node node;
...
int MpvVideoController::getPropertyAsync(const QString &property, int id)
{
    return mpv_get_property_async(m_mpv, id, property.toUtf8().constData(), MPV_FORMAT_NODE);
}
